(function (win, $, doc) {
    ...
    win.HYFRESH[pluginName] = function (container, args) {
        ...
        var defParams = {
            effect : 'default', // Could be 'default', 'fade', 'slide', 'flip', 'layerSlide'
            layerWrapElements : container,
            layerElements : '.hive-layer',
            layerContentElements : '.hive-layer-content',
            layerFixedElements : '.hive-layer-fixed',
            layerFixedTopElements : '.hive-layer-top-fixed',
    ...
    win.HYFRESH[pluginName].prototype = {
        ...
        offsetFunc : function (value, index) {
            value.layerWrap.find('.common-layer__wrapper').scrollTop(value.layerWrap.find('.common-layer__content > p').eq(index).position().top)

            // value.layerWrap.find('.common-layer__wrapper').stop().animate({
            //   scrollTop: value.layerWrap.find('.common-layer__content > p').eq(index).position().top
            // }, 100)
        },
        onLayerOpen : function (e) {
            ...
            if ($(e.target).data("offset")) {
                $.map($("."+Array.prototype.slice.call(e.target.classList).join('.') + "[data-offset='true']"), function (value, i) {
                    if (value === e.target) {
                        indexOffset = i;
                    }
                })
        ...
        for (var i = 0, max = cmSLayer.length; i < max; i++) {
            (function (index) {
                var _this = cmSLayer.eq(i),
                    ...
                    _effect = cmSLayer.eq(i).data('effect');
                new window.HYFRESH[pluginName](_layer, {
                    effect: _effect,
