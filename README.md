...
* 슬라이드 객체가 만들어질 때 코드가 길어지고 그러다보면 코드 실행순서가 엉켜버려서  
  (스와이퍼가 적용되고 나서 CSS랜더링이 되고난 후 값을 읽어와야되는데, CSS가 제대로 렌더링되기전에 값을 읽어와 값을 잘못 가져오는 경우가 발생하기도 한다.)  
  이 때문에 `slideTo` 같은 메소드를 사용하면 좀 오차있는 위치로 가기도 함
  
    * 이건 코드 실행순서를 정확히 파악해 코드 자체를 수정하는 수밖에없음
