...
export const watchingResources = (done) => {
    ...
    watch(config.imgSetting.src, series(swipeImage, spriteSvg, cloneSvgSprites, removeTempSvgSprites, generateImages, swipeCSS, concatLibsCSS, generateSprite, compileSCSS, browserSyncReload));
    watch(config.htmlSetting.src, series(swipeHTML, setHTML, generateHTML, browserSyncReload));
    watch(config.cssSetting.src, series(swipeCSS, concatLibsCSS, generateSprite, compileSCSS, browserSyncReload));
    done();
}
